 ...
 */
define('brand-pathname-placeholder/views/catalog-cover', [
    'underscore',
    'jquery',
    'util/Util',
    'brand-pathname-placeholder/models/product',
    'views/BaseView',
    'brand-pathname-placeholder/views/product-callout'
    ...
    var CatalogCoverView = BaseView.extend(/** @lends CatalogCoverView.prototype */ {
         ...
         * @type {String}
         */
        className: 'geo-CatalogCoverView',
        ...
        _handleToggleCallout: function(ev) {
            var $openCallout = this.$el.find('.geo-ProductCalloutView:visible');
        ...
        _template: function(catalog) {
            var tpl = _.template(
                '<div class="geo-CatalogView-background" style="background-image: url(<%= imagePath %>); "></div>'
            );
            return tpl({
